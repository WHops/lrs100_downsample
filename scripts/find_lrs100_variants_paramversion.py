...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Find LRS100 variants with configurable parameters.")

    # Adding arguments with default values
    parser.add_argument('--snv_pos_leniency', type=int, default=snv_pos_leniency, help='SNV position leniency (default: 1)')
    parser.add_argument('--indel_pos_leniency', type=int, default=indel_pos_leniency, help='Indel position leniency (default: 10)')
    parser.add_argument('--indel_size_leniency', type=int, default=indel_size_leniency, help='Indel size leniency (default: 5)')
    parser.add_argument('--sv_overlap_minpct', type=float, default=sv_overlap_minpct, help='SV overlap minimum percentage (default: 0.5)')
    parser.add_argument('--bnd_pos_leniency', type=int, default=bnd_pos_leniency, help='BND position leniency (default: 50)')
    parser.add_argument('--str_pos_leniency', type=int, default=str_pos_leniency, help='STR position leniency (default: 1000)')
    parser.add_argument('--str_cn_fraction_leniency', type=int, default=str_cn_fraction_leniency, help='STR copy number fraction leniency (default: 20)')
    parser.add_argument('--mt_percentpoints_leniency', type=int, default=mt_percentpoints_leniency, help='Mitochondrial percent points leniency (default: 20)')

    parser.add_argument('input_variants_file', type=str, help='Input variants file')
    parser.add_argument('run_basedir', type=str, help='Run base directory')
    parser.add_argument('output_file', type=str, help='Output file')
    parser.add_argument('permutation_index', type=str, nargs='?', default=None, help='Permutation index (optional)')

    return parser.parse_args()

def set_globals_from_args(args):
    global snv_pos_leniency, indel_pos_leniency, indel_size_leniency
    global sv_overlap_minpct, bnd_pos_leniency
    global str_pos_leniency, str_cn_fraction_leniency, mt_percentpoints_leniency

    # Set the global variables based on parsed arguments
    snv_pos_leniency = args.snv_pos_leniency
    indel_pos_leniency = args.indel_pos_leniency
    indel_size_leniency = args.indel_size_leniency
    sv_overlap_minpct = args.sv_overlap_minpct
    bnd_pos_leniency = args.bnd_pos_leniency
    str_pos_leniency = args.str_pos_leniency
    str_cn_fraction_leniency = args.str_cn_fraction_leniency
    mt_percentpoints_leniency = args.mt_percentpoints_leniency
...
def condition_start_within_leniency(target_start, variant_start, leniency):
    #pdb.set_trace()
    if (abs(target_start - (variant_start)) < 3):
        print(target_start - variant_start)
...
def condition_inslen_within_leniency_using_ref_alt(target_inslen, variant, leniency):
    if abs((len(variant.ALT[0]) - len(variant.REF))  - target_inslen) <= leniency:
        return True
    return False
...
def condition_svlen_within_leniency_using_ref_alt(
    ...
    if (
        abs(abs(len(variant.REF) - len(variant.ALT[0])) - ((target_end+1) - target_start))
        <= leniency
    ):
        return True
    return False
...
def search_hificnv(target_variant, vcf_paths, sv_overlap_minpct):
    ...
    for start, end in merged_intervals:
        if reciprocal_overlap(target_start, target_end, start, end) >= sv_overlap_minpct:
    ...
    if target_vartype == "INS":
        ...
        for variant in vcf_reader:
            if (
                ...
                and condition_start_within_leniency(
                    target_pos1, variant.start+1, bnd_pos_leniency
                ) and (
...
def main(input_variants, run_basedir, output_file, permutation_index=None):
    ...
    for index, variant in variants.iterrows():
        ...
        if source == "snv":
            ...
            result = search_hificnv(variant, infile_paths, sv_overlap_minpct)
        elif source == "para":
            result = search_para(
                variant,
                infile_paths,
...
if __name__ == "__main__":
    ...
    # Call main function with necessary arguments
    main(
        args.input_variants_file,
        args.run_basedir,
        args.output_file,
        args.permutation_index
    )
